// apps\frontend\src\components\admin\AdminFields.tsx
import type {
...
export function Field({ label, children, hint }: FieldProps) {
  return (
    <label className="grid gap-2 text-sm font-semibold text-[#3c232c]/75">
      ...
      {children}
      {hint && (
        <span className="text-xs font-medium text-[#3c232c]/50">{hint}</span>
      )}
    </label>
  );
}
...
export function AdminButton({
  ...
  const variants = {
    primary:
      "border-transparent bg-gradient-to-r from-[#ad6a6c] to-[#3c232c] text-white shadow-md hover:opacity-90",
    secondary:
      "border-[#efdad0] bg-white/70 text-[#3c232c] hover:border-[#ad6a6c] hover:bg-white",
    danger:
      "border-red-200 bg-red-50 text-red-700 hover:border-red-300 hover:bg-red-100",
    ghost:
      "border-transparent bg-transparent text-[#3c232c]/70 hover:bg-white/60 hover:text-[#3c232c]",
  };
