...
export function Stats() {
  ...
  return (
    <section className="relative bg-[#f9f6f3] px-4 py-10 sm:px-6 lg:px-8">
      {/* Background Glow */}
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative rounded-xl border border-[#e3d1d1]/60 bg-white/70 px-4 py-5 text-center shadow-[0_6px_16px_rgba(60,35,44,0.06)] backdrop-blur-md transition-transform duration-300 hover:-translate-y-1 hover:bg-white"
            >
              {/* Hover Glow */}
              ...
              <div className="mx-auto mb-2 flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-[#ad6a6c] to-[#3c232c] shadow-md">
                <Icon
                  ...
                  className="text-[18px] text-[#f8e7de]"
                />
              </div>
              ...
              {/* Label */}
              <p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#7d5a61] sm:text-xs">
                {stat.label}
              </p>
