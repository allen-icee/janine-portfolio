// apps\frontend\src\pages\AdminLogin.tsx
...
export function AdminLogin() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isSupabaseConfigured || !supabase) {
      setError(
        "Supabase is not configured yet. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY first.",
      );
      return;
    }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f9f6f3] px-4 py-12">
      <section className="w-full max-w-md rounded-[2rem] border border-[#efdad0] bg-white/70 p-6 shadow-2xl backdrop-blur">
        <Link
          to="/"
          className="mb-8 inline-block text-sm text-[#3c232c]/58 transition hover:text-[#3c232c]"
        >
          Back to website
        </Link>
        ...
        <p className="mt-3 leading-7 text-[#3c232c]/64">
          No admin password is stored in this website. Login uses Supabase Auth,
          then checks your account against the approved admin list.
        </p>
        {!isSupabaseConfigured && (
          <p className="mt-5 rounded-xl border border-[#efdad0] bg-[#f9f6f3] p-3 text-sm text-[#ad6a6c]">
            Supabase is not connected yet, so admin login is intentionally
            disabled.
          </p>
        )}
