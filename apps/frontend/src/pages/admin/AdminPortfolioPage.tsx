...
export function AdminPortfolioPage() {
  ...
  const [categoryRows, setCategoryRows] =
    useState<CategoryRow[]>(defaultCategoryRows);
  ...
  const updateForm = <Key extends keyof PortfolioRow>(
    key: Key,
    value: PortfolioRow[Key],
  ) => {
    setForm((current) => ({ ...current, [key]: value }));
    setIsDirty(true);
  };

  const updateCategoryForm = <Key extends keyof CategoryRow>(
    key: Key,
  ...
  const openCreateProject = () => {
    setForm({
      ...emptyPortfolio,
      category: categoryNames[0] ?? "Research",
  ...
  const saveProject = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(
        form.id ? "Portfolio item updated." : "Portfolio item created.",
      );
      closeProjectModal();
      await loadItems();
    } catch (error) {
  ...
  return (
    <AdminGuard>
      <AdminShell
        title="Portfolio"
        description="Create project cards, manage category tabs, and upload cover images."
      >
        ...
        <section className="mb-8 rounded-2xl border border-[#efdad0] bg-white/50 p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="font-serif text-2xl font-bold text-[#3c232c]">
              Categories
            </h2>
            <p className="text-sm text-[#3c232c]/55">
              {categoryRows.length} tabs
            </p>
          </div>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {categoryRows.map((category) => (
              <article
                ...
                className="flex items-center justify-between gap-3 rounded-xl border border-[#efdad0] bg-white/70 p-3"
              >
                <div>
                  <p className="font-semibold text-[#3c232c]">
                    {category.name}
                  </p>
                  <p className="text-xs text-[#3c232c]/50">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="grid size-9 place-items-center rounded-full text-[#3c232c]/70 hover:bg-[#f8cdb4]/25"
                  >
                    <Edit3 size={15} />
                  </button>
                  {category.id && (
        ...
        <div className="grid gap-4">
          ...
          {visibleItems.map((item) => (
            <article
              key={item.id}
              className="grid gap-4 rounded-2xl border border-[#efdad0] bg-white/60 p-4 shadow-sm sm:grid-cols-[160px_1fr]"
            >
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <h2 className="mt-1 font-serif text-2xl font-bold text-[#3c232c]">
                    {item.title}
                  </h2>
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-[#3c232c]/65">
                    {item.summary}
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {(item.technologies ?? []).map((tool) => (
                      <span
                        key={tool}
                        className="rounded-full bg-[#f8cdb4]/30 px-3 py-1 text-xs font-semibold text-[#3c232c]/75"
                      >
                        {tool}
                      </span>
                    ))}
          ...
          <form onSubmit={saveProject} className="grid gap-5">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Field
                ...
                hint="Optional if uploading an image below."
              >
                <TextInput
                  value={form.cover_url}
                  onChange={(event) =>
                    updateForm("cover_url", event.target.value)
                  }
                />
              </Field>
            </div>
            ...
            <Field label="Upload Cover Image">
              <div className="rounded-2xl border border-dashed border-[#ad6a6c]/35 bg-white/50 p-4">
                ...
                />
                {imageFile && (
                  <p className="mt-3 text-sm text-[#3c232c]/60">
                ...
                {form.cover_url && !imageFile && (
                  <img
                    src={form.cover_url}
                    alt="Current cover"
                    className="mt-4 h-36 rounded-xl object-cover"
                  />
                )}
              </div>
            </Field>
            ...
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <AdminButton
                type="button"
                variant="secondary"
                onClick={requestCloseProjectModal}
                disabled={isSaving}
              >
                Cancel
              </AdminButton>
          ...
          <form onSubmit={saveCategory} className="grid gap-5">
            ...
            <Field label="Sort Order">
              <TextInput
                ...
                onChange={(event) =>
                  updateCategoryForm("sort_order", Number(event.target.value))
                }
              />
            </Field>
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <AdminButton
                type="button"
                variant="secondary"
                onClick={requestCloseCategoryModal}
                disabled={isSaving}
              >
                Cancel
              </AdminButton>
